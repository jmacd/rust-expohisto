...
//! Downscale and widen operations.
//!
//! Downscale merges groups of `2^change` adjacent buckets by summing
//! their counters. The algorithm has two phases:
//!
//! **Phase 1 — SWAR-widen-and-max:** widens packed counters in place
//! by `swar_steps = min(change, log2(spw))` doublings and tracks the
//! maximum lane value (for width selection).
//!
//! **Phase 2 — Dispatch:**
//! - *Case A (intra-word):* `swar_steps == change`. All group sums
//!   are SWAR lanes. Repack in place at the target width.
//! - *Case B (inter-word):* `swar_steps < change`. After SWAR to U64,
//!   groups of consecutive words are summed in place, then repacked.

use super::Histogram;
use super::swar::swar_widen_max;
...
impl<const N: usize> Histogram<N> {
    ...
    fn downscale_case_b(
        ...
        if width == Width::U64 {
            ...
            let start_slot =
                (self.index_start - self.index_base).rem_euclid(N as i32) as usize;
            if start_slot != 0 {
        ...
        let first_word =
            (new_start * (1i32 << change) - self.index_base) / spw;
