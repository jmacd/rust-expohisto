...
├── histogram.rs     # Core histogram (~4500 lines): struct, update, merge,
│                    # downscale, widen, literal mode, BucketView
├── mapping.rs       # Scale-to-index mapping dispatch
├── precision.rs     # P32/P64 precision tiers (compile-time)
├── exponent.rs      # Scale ≤ 0 mapping (bit extraction)
├── float64.rs       # IEEE 754 bit manipulation helpers
├── logarithm.rs     # ln()-based mapping algorithm
├── lookup.rs        # Lookup table data (generated by build.rs)
...
- **Allocation-free**: `Histogram<N, P>` is entirely stack-allocated. The `N`
