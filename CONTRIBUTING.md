...
Three fuzz targets exist for finding edge-case bugs:
...
├── histogram/       # Core histogram: struct, update, merge,
│                    # downscale, widen, literal mode, BucketView
...
├── logarithm.rs     # ln()-based mapping algorithm (always available)
...
- **Allocation-free**: `Histogram<N>` is entirely stack-allocated. The `N`
  words of data pool hold both MMSC statistics and bucket data.
...
- **Compile-time table generation**: The lookup table and its scale (`scale-1`..`scale-20`)
  are Cargo features — no runtime dispatch overhead. The built-in `logarithm`
